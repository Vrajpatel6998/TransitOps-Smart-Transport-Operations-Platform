...
import { MaintenanceLog } from '../../utils/types';
...
export function Maintenance() {
  const { data: logs, loading, error, refresh } = useGetData<MaintenanceLog[]>(apiUrls.maintenance);
  ...
  const openCreateModal = () => {
    ...
    setStatus('active');
    setFormError(null);
    setModalTitle('Log Maintenance Service');
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (log: MaintenanceLog) => {
    ...
    setFormError(null);
    setModalTitle(`Edit Maintenance Log #${log.id}`);
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!vehicleId || !description.trim() || !cost || !date) {
    ...
    if (isNaN(Date.parse(date))) {
      setFormError('Please provide a valid log date');
      return;
    }
    ...
    try {
      ...
      if (editingLog) {
        await apiClient.put(`${apiUrls.maintenance}/${editingLog.id}`, payload);
      } else {
        await apiClient.post(apiUrls.maintenance, payload);
      }
    ...
    <div className="space-y-6">
      {error && (
        <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 flex items-center gap-3">
          ...
          <span>Error loading maintenance logs: {error}</span>
        </div>
      )}

      {/* Title & Action Bar */}
      <div className="flex justify-between items-center bg-slate-900 p-4 border border-slate-800 rounded-2xl shadow-sm">
        <p className="text-sm text-slate-400">Track and schedule servicing jobs, costs, descriptions, and statuses for fleet assets.</p>
              ...
              {logs && logs.length > 0 ? (
                logs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="py-4 px-6 max-w-xs truncate">{log.description}</td>
                    <td className="py-4 px-6 text-rose-400 font-semibold">${Number(log.cost).toLocaleString()}</td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalTitle}>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Description / Issues Fixed</label>
            <textarea 
              required
              rows={3}
              value={description}
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-sm text-slate-100 focus:outline-none focus:border-emerald-500"
              placeholder="Describe oil change, brake check, tire repair..."
            />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Service Cost (USD)</label>
              <input 
                type="number" 
                required
                value={cost}
